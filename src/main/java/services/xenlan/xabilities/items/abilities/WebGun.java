...
public class WebGun implements Listener {
    ...
    public void onLaunch(ProjectileLaunchEvent e) {
        ...
        if (entity instanceof Snowball) {
            ...
            snowball.setMetadata("webgun", new FixedMetadataValue(xAbilities.getInstance(), player.getUniqueId()));
        }
    }
    ...
    public void onClick(PlayerInteractEvent event) {
        ...
        if (Cooldowns.getWebgun().onCooldown(player)) {
            for (String list : xAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                player.sendMessage(chatUtil.chat(list.replace("%ability%", xAbilities.config.getConfig().getString("WebGun.Name")).replace("%time%", Cooldowns.getWebgun().getRemaining(player))));
                event.setCancelled(true);
                player.updateInventory();
                return;
            }
        ...
        if (Cooldowns.getRefundcool().onCooldown(player)) {
            for (String list : xAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                player.sendMessage(chatUtil.chat(list.replace("%ability%", xAbilities.config.getConfig().getString("Anti-Glitch.Cooldown-Display")).replace("%time%", Cooldowns.getRefundcool().getRemaining(player))));
                event.setCancelled(true);
                player.updateInventory();
                return;
            }
        }
        for (String list : xAbilities.config.getConfig().getStringList("WebGun.Message.Used")) {
            player.sendMessage(chatUtil.chat(list));
        }
        Cooldowns.getWebgun().applyCooldown(player, xAbilities.config.getConfig().getInt("WebGun.Cooldown") * 1000);
    ...
    public void onThrow(ProjectileHitEvent event) {
        ...
        if (xAbilities.config.getConfig().getBoolean("WebGun.3x3-Spawn-Above-Hit-Block")) {
    ...
    public void onBlockChange(EntityChangeBlockEvent e) {
        ...
        if (e.getEntity().getType() == EntityType.FALLING_BLOCK && e.getTo() == Material.WEB) {
            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(xAbilities.getInstance(),
                    new BukkitRunnable() {
                public void run() {
                    e.getBlock().setType(Material.AIR);
                }
            }, 20 * xAbilities.config.getConfig().getInt("WebGun.Web-Delay"));
        }
    }
