    ...
    public void onNinjaStar(PlayerInteractEvent event) {
        ...
        if (Cooldowns.getNinjastar().onCooldown(p)) {
            for (String list : xAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                p.sendMessage(chatUtil.chat(list.replace("%ability%", xAbilities.config.getConfig().getString("NinjaStar.Name")).replace("%time%", Cooldowns.getNinjastar().getRemaining(p))));
                event.setCancelled(true);
                p.updateInventory();
                return;
            }
        ...
        }.runTaskLater(xAbilities.getInstance(), xAbilities.config.getConfig().getInt("NinjaStar.WarmUp") * 20);

        for (String list : xAbilities.config.getConfig().getStringList("NinjaStar.Message.Teleporting")) {
            p.sendMessage(chatUtil.chat(list).replaceAll("%player%", damaged.getName()));
        }
        ninja.remove(damaged);
        Cooldowns.getNinjastar().applyCooldown(p, xAbilities.config.getConfig().getInt("NinjaStar.Cooldown") * 1000);
