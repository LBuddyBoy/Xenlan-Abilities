...
public class TeleportBow implements Listener {
    ...
    public void SwitcherThrown(PlayerInteractEvent event) {
        ...
        if (Cooldowns.getTelebow().onCooldown(player)) {
            for (String list : xAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                player.sendMessage(chatUtil.chat(list.replace("%ability%", xAbilities.config.getConfig().getString("TeleBow.Name")).replace("%time%", Cooldowns.getTelebow().getRemaining(player))));
                event.setCancelled(true);
                player.updateInventory();
                return;
            }
        ...
        Cooldowns.getTelebow().applyCooldown(player, xAbilities.config.getConfig().getInt("TeleBow.Cooldown") * 1000);
    ...
    public void onLaunch(ProjectileLaunchEvent e) {
        ...
        if (entity instanceof Arrow) {
            ...
            arrow.setMetadata("telebow", new FixedMetadataValue(xAbilities.getInstance(), player.getUniqueId()));
        ...
        if (e.getEntity() instanceof Player) {
            if (e.getDamager() instanceof Arrow) {
                ...
                if (arrow.getShooter() instanceof Player) {
                    ...
                    for (String list : xAbilities.config.getConfig().getStringList("TeleBow.Message.Hit-Someone")) {
