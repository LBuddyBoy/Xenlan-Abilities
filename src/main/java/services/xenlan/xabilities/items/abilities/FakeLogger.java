    ...
    public void onClick(PlayerInteractEvent event) {
        ...
        if (item.isSimilar(event.getPlayer().getItemInHand())) {
            ...
            if (Cooldowns.getFakelogger().onCooldown(player)) {
                for (String list : xAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                    player.sendMessage(chatUtil.chat(list.replace("%ability%", xAbilities.config.getConfig().getString("FakeLogger.Name")).replace("%time%", Cooldowns.getFakelogger().getRemaining(player))));
                    event.setCancelled(true);
                    player.updateInventory();
                    return;
                }
            ...
            double health = xAbilities.config.getConfig().getDouble("FakeLogger.Villager.Health");
            String name = xAbilities.config.getConfig().getString("FakeLogger.Villager.Name");
            int time = xAbilities.config.getConfig().getInt("FakeLogger.Player.Invis-Time");

            Villager villager = (Villager) event.getPlayer().getWorld().spawnEntity(event.getPlayer().getLocation(), EntityType.VILLAGER);

            villager.setMetadata("logger", new FixedMetadataValue(xAbilities.getInstance(), player.getName()));
            ...
            Cooldowns.getFakelogger().applyCooldown(player, xAbilities.config.getConfig().getInt("FakeLogger.Cooldown") * 1000);
            ...
            }.runTaskLater(xAbilities.getInstance(), 20 * time);
            ...
            }.runTaskLater(xAbilities.getInstance(), 20 * 30);
            event.setCancelled(true);
        }
